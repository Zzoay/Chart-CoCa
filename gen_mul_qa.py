...
if __name__ == '__main__':
    ...
    # create pipeline
    pipe = pipeline(model, backend_config=TurbomindEngineConfig(session_len=8192, tp=torch.cuda.device_count(), cache_max_entry_count=0.8))
    ...
    for i in tqdm(range(0, len(data_ids), batch_size)):
        ...
        batch_data_ids_to_check, batch_images_to_check = [], []
        for data_id in batch_ids:
            if os.path.exists(f'charts/{data_id}_aj.jpg'):
                batch_data_ids_to_check.append(data_id)
                batch_images_to_check.append(load_image(f'charts/{data_id}_aj.jpg'))
        assert len(batch_data_ids_to_check) == len(batch_images_to_check)
        ...
        direct_answers = []
        for data_id, info, img in zip(batch_data_ids_to_check, extract_info, batch_images_to_check):
            ...
            for q_idx, q in enumerate(q_list):
                ...
                # get answer
                if "title" in q:
                    tmp_ans_short[q_list.index(q)] += f"{specific_info['title']}" if len(specific_info['title']) > 1 and str(specific_info['title']).strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                elif "label of the x-axis" in q:
                    label_of_xaxis = specific_info['label_of_xaxis']
                    tmp_ans_short[q_list.index(q)] += f"{label_of_xaxis}" if len(label_of_xaxis) > 1 and str(label_of_xaxis).strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                elif "label of the y-axis" in q:
                    label_of_yaxis = specific_info['label_of_yaxis']
                    tmp_ans_short[q_list.index(q)] += f"{label_of_yaxis}" if len(label_of_yaxis) > 1 and str(label_of_yaxis).strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                elif "leftmost labeled tick on the x-axis" in q:
                    x_tick_leftmost = str(specific_info['x_tick_leftmost'])
                    tmp_ans_short[q_list.index(q)] += f"{x_tick_leftmost}" if x_tick_leftmost.strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                elif "rightmost labeled tick on the x-axis" in q:
                    x_tick_rightmost = str(specific_info['x_tick_rightmost'])
                    tmp_ans_short[q_list.index(q)] += f"{x_tick_rightmost}" if x_tick_rightmost.strip() not in ['Not Applicable', 'None', ''] else "Not Applicable" 
                elif "highest labeled tick on the y-axis" in q:
                    y_tick_highest = str(specific_info['y_tick_highest'])
                    tmp_ans_short[q_list.index(q)] += f"{y_tick_highest}" if y_tick_highest.strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                elif "lowest labeled tick on the y-axis" in q:
                    y_tick_lowest = str(specific_info['y_tick_lowest'])
                    tmp_ans_short[q_list.index(q)] += f"{y_tick_lowest}" if y_tick_lowest.strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                    ...
                    tmp_ans_short[q_list.index(q)] += f"{x_tick_difference}" if x_tick_difference != "Not Applicable" else "There is no numerical tick labels on the x-axis. Thus, the answer is 'Not Applicable'."
                    ...
                    tmp_ans_short[q_list.index(q)] += f"{y_tick_difference}" if y_tick_difference != "Not Applicable" else "There is no numerical tick labels on the y-axis. Thus, the answer is 'Not Applicable'."
                elif "discrete labels are there in the legend" in q:
                    num_of_label_in_legend = len(specific_info['labels_in_legend'])
                    tmp_ans_short[q_list.index(q)] += f"{num_of_label_in_legend}" if isinstance(labels_in_legend, list) and num_of_label_in_legend > 0 else "Not Applicable"
                elif "the names of the labels in the legend" in q:
                    tmp_ans_short[q_list.index(q)] += f"{', '.join(labels_in_legend)}" if isinstance(labels_in_legend, list) and len(labels_in_legend) > 0 else "Not Applicable"
                    ...
                    if axes_num == 2:
                        tmp_ans_short[q_list.index(q)] += f"{ticks_num}" if specific_info['num_ticks_xaxis'] > 0  or specific_info['num_ticks_yaxis'] > 0 else "Not Applicable"
                    else:
                        tmp_ans_short[q_list.index(q)] += f"{ticks_num}" if specific_info['num_ticks_xaxis'] > 0  or specific_info['num_ticks_yaxis'] > 0 or specific_info['num_ticks_zaxis'] > 0 else "Not Applicable"
                elif "layout of the subplots" in q:
                    if num_rows == 1 and num_cols == 1:
                        tmp_ans_short[q_list.index(q)] += f"1 by 1"
                    else:
                        tmp_ans_short[q_list.index(q)] += f"{num_rows} by {num_cols}"
                elif "the number of subplots" in q:
                    if num_rows == 1 and num_cols == 1:
                        tmp_ans_short[q_list.index(q)] += "1"
                    else:
                        tmp_ans_short[q_list.index(q)] += str(num_total)
                elif "colorbar" in q:
                    # default
                    if "difference between the maximum and minimum values" in q and specific_info['colorbar_diff'] != "Not Applicable":
                        ...
                        tmp_ans_short[q_list.index(q)] += colorbar_diff
                        ...
                        tmp_ans_short[q_list.index(q)] += colorbar_max if colorbar_max != "Not Applicable" else "Not Applicable"
                    else:
                        tmp_ans_short[q_list.index(q)] += "Not Applicable"
                elif "how many lines" in q:
                    if specific_info['num_lines'] == "Not Applicable":
                        tmp_ans_short[q_list.index(q)] += "Not Applicable"
                    else:
                        tmp_ans_short[q_list.index(q)] += str(specific_info['num_lines'])
                elif "any lines intersect" in q:
                    if specific_info['intersection_of_lines'] == "Not Applicable":
                        tmp_ans_short[q_list.index(q)] += "Not Applicable"
                    else:
                        tmp_ans_short[q_list.index(q)] += f"Yes" if specific_info['intersection_of_lines'] == 'Yes' else "No"
                elif "general trend of data from left to right" in q:
                    if specific_info['trend_of_lines'] == "Not Applicable":
                        tmp_ans_short[q_list.index(q)] += "Not Applicable"
                    else:
                        tmp_ans_short[q_list.index(q)] += str(specific_info['trend_of_lines'])

                
                if "title" in q:
                    tmp_ans[q_list.index(q)] += f"The title is: {specific_info['title']}" if len(specific_info['title']) > 1 and str(specific_info['title']).strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                elif "label of the x-axis" in q:
                    label_of_xaxis = specific_info['label_of_xaxis']
                    tmp_ans[q_list.index(q)] += f"The label of the x-axis is: {label_of_xaxis}" if len(label_of_xaxis) > 1 and str(label_of_xaxis).strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                elif "label of the y-axis" in q:
                    label_of_yaxis = specific_info['label_of_yaxis']
                    tmp_ans[q_list.index(q)] += f"The label of the y-axis is: {label_of_yaxis}" if len(label_of_yaxis) > 1 and str(label_of_yaxis).strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                elif "leftmost labeled tick on the x-axis" in q:
                    x_tick_leftmost = str(specific_info['x_tick_leftmost'])
                    tmp_ans[q_list.index(q)] += f"The leftmost labeled tick on the x-axis is: {x_tick_leftmost}." if x_tick_leftmost.strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                elif "rightmost labeled tick on the x-axis" in q:
                    x_tick_rightmost = str(specific_info['x_tick_rightmost'])
                    tmp_ans[q_list.index(q)] += f"The rightmost labeled tick on the x-axis is: {x_tick_rightmost}." if x_tick_rightmost.strip() not in ['Not Applicable', 'None', ''] else "Not Applicable" 
                elif "highest labeled tick on the y-axis" in q:
                    y_tick_highest = str(specific_info['y_tick_highest'])
                    tmp_ans[q_list.index(q)] += f"The highest labeled tick on the y-axis is: {y_tick_highest}." if y_tick_highest.strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                elif "lowest labeled tick on the y-axis" in q:
                    y_tick_lowest = str(specific_info['y_tick_lowest'])
                    tmp_ans[q_list.index(q)] += f"The lowest labeled tick on the y-axis is: {y_tick_lowest}." if y_tick_lowest.strip() not in ['Not Applicable', 'None', ''] else "Not Applicable"
                elif "difference between consecutive numerical tick values on the x-axis" in q:
                    x_tick_difference = str(specific_info['x_tick_difference'])
                    ...
                    tmp_ans[q_list.index(q)] += f"The labels in the legend are: {', '.join(labels_in_legend)}." if isinstance(labels_in_legend, list) and len(labels_in_legend) > 0 else "Not Applicable"
                elif "total number of explicitly labeled ticks across all axes" in q:
                    ticks_num = str(specific_info['num_ticks_all_axes'])
                    axes_num = 2 if specific_info['num_ticks_zaxis'] == 0 else 3
            ...
            one_image = [img] * 4  * repeat_sample_num
            batch_prompts = [(f"<image>\n{q}", img) for img, q in zip(one_image, q_list)] * repeat_sample_num

            batch_responses = pipe(batch_prompts, gen_config=GenerationConfig(max_new_tokens=256, do_sample=True, top_p=0.6))
            batch_answers = [response.text for response in batch_responses]

            direct_answers.append("\n".join(tmp_ans))  # ground truth answers

            with open(os.path.join(f'mul_qas/{data_id}.jsonl'), 'w') as f:
                for q_idx, (q, gt, gt_short) in enumerate(zip(q_list, tmp_ans, tmp_ans_short)):
                    res = {'question': q}
                    for repeat_idx in range(repeat_sample_num):
                        res[f"answer_{repeat_idx}"] = batch_answers[repeat_idx*len(q_list)+q_idx]
                    res['gt_answer'] = gt
                    res['gt_answer_short'] = gt_short
